...
create table if not exists public.staff_directory (
  ...
  email text,
  repairdesk_user_id bigint,
  job_role text,
  default_store_id bigint references public.store_locations(id) on delete set null,
  active boolean not null default true,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
with staff_seed (display_name, email, repairdesk_user_id, job_role, default_store_code, active) as (
  values
    ('Andy', 'shangzelin2001@gmail.com', 37564::bigint, 'technician', 'parkridge', true),
    ('Anna', 'Ana04maria14@gmail.com', 82237::bigint, 'store_manager', 'northlakes', true),
    ('Bonnie', 'bonniechiu1212aabb@gmail.com', 69850::bigint, 'store_manager', 'fairfield', true),
    ('Bowen', null, null::bigint, 'admin', null, true),
    ('Fiona', 'yingmccoy@gmail.com', 75726::bigint, 'store_manager', 'toowong', true),
    ('Henry Ang', 'henryang98@gmail.com', null::bigint, null, null, false),
    ('JANAPHY', 'janaphy0530@hotmail.com', null::bigint, null, null, false),
    ('Jinny', 'gksmftka@gmail.com', 80470::bigint, 'technician', 'fairfield', true),
    ('Joanna Chen', '824195774@qq.com', 48578::bigint, 'store_manager', 'parkridge', true),
    ('Steven T', 'steven_troya@hotmail.com', null::bigint, null, null, false)
)
...
select
  ...
  stores.id,
  staff.active
from staff_seed staff
left join public.store_locations stores on stores.store_code = staff.default_store_code
...
-- Unified POS invoices, repair workflow, search, and refunds.
...
-- Brisbane business-date filtering for the unified Invoice History.
...
-- Temporary POS stocktake access is controlled per active staff member.
...
begin
  ...
  if selected_staff.id is null then
    return jsonb_build_object('ok', false, 'enabled', false, 'message', 'Active staff member not found.');
  end if;
...
-- Individual staff email/password login and first-login POS PIN setup.
-- Keep this section aligned with migration 20260820145225_add_staff_email_first_login_credentials.sql.
alter table public.staff_directory
...
-- The former shared front-end password is no longer part of the staff login flow.
...
-- Keep the monolithic bootstrap schema aligned with the indexed staff-session
-- and report-setup optimization migration (20260821085912).
...
-- POS warranty claims (2026-09-08)
...
-- Repair activity audit and searchable archived cards (2026-09-09).
...
-- New repair intake is committed with its first customer signature. Existing
-- tickets continue to use save_pos_repair_card_signature for re-signing.
...
declare
  ...
  ticket_row public.pos_repair_tickets%rowtype;
begin
  if jsonb_typeof(ticket_payload) <> 'object' then raise exception 'Ticket payload must be an object'; end if;
  if jsonb_typeof(signature_payload) <> 'object' then raise exception 'Signature payload must be an object'; end if;
  ticket_code_value := coalesce(nullif(btrim(ticket_payload->>'ticket_code'), ''), nullif(btrim(ticket_payload->>'id'), ''));
  ...
  actor := public.pos_authorized_actor(
    session_token,
    coalesce(ticket_payload->>'store_code', ticket_payload->>'store_db_code', ticket_payload->>'storeId', ticket_payload->>'store_id'),
  ...
  if exists (select 1 from public.pos_repair_tickets ticket where ticket.ticket_code = ticket_code_value) then
    raise exception 'A repair ticket with this number already exists';
  end if;

  normalized_ticket := ticket_payload || jsonb_build_object(
    'id', ticket_code_value, 'ticket_code', ticket_code_value,
    'store_id', actor->>'store_code', 'storeId', actor->>'store_code',
    'store_code', actor->>'store_code', 'store_db_code', actor->>'store_code',
    'staff_name', actor->>'staff_name', 'createdBy', actor->>'staff_name',
    'updatedBy', actor->>'staff_name', 'created_by', actor->>'staff_name',
    'updated_by', actor->>'staff_name'
  );
  ...
  select * into ticket_row from public.pos_repair_tickets ticket where ticket.ticket_code = ticket_code_value;
  return jsonb_build_object(
    'ok', true,
    'ticket', public.pos_repair_ticket_payload(ticket_row),
...
revoke all on function public.create_pos_repair_ticket_with_signature(text, jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.create_pos_repair_ticket_with_signature(text, jsonb, jsonb) to service_role;
...
-- Admin sales drill-down: paginated sales/refund movements behind the overview totals.
...
begin
  ...
  if category_value <> all (array['all','repair','mis','product','other']::text[]) then
    raise exception 'Sales category is invalid';
  end if;

  with selected_stores as materialized (
    select store.id, store.store_code, store.store_name from public.store_locations store
    where store.active and store.store_code = any (array['parkridge','fairfield','northlakes','toowong']::text[])
      and (store_value = '' or store.store_code = store_value)
  ),
  sale_line_movements as materialized (
    select 'sale'::text event_type, 'sale:'||sales_order.id::text event_key,
      sales_order.created_at transaction_at, sales_order.business_date transaction_date,
      store.store_code, store.store_name, sales_order.invoice_number, sales_order.order_code,
      ''::text refund_code, ''::text reason, sales_order.customer_name, sales_order.customer_phone,
      sales_order.customer_email, sales_order.staff_name, sales_order.payment_method, sales_order.payment_status,
      sales_order.total invoice_total, sales_order.amount_paid,
      greatest(sales_order.total-sales_order.amount_paid,0) balance_due,
      case when sales_line.line_type='repair' then 'repair' when sales_line.line_type='used_device' then 'mis'
        when sales_line.line_type in ('product','retail') then 'product' else 'other' end category_key,
      sales_line.id line_id, sales_line.name item_name, sales_line.sku, sales_line.quantity,
      sales_line.unit_price, sales_line.line_total amount, coalesce(repair_ticket.ticket_code,'') ticket_code,
      coalesce(nullif(trim(sales_line.line_payload->>'note'),''),nullif(trim(sales_line.line_payload->>'notes'),''),
        nullif(trim(sales_line.line_payload->>'description'),''),'') line_note
    from public.pos_sales_orders sales_order join selected_stores store on store.id=sales_order.store_id
  ...
  refund_line_movements as materialized (
    select 'refund'::text event_type, 'refund:'||refund.id::text event_key,
      refund.created_at transaction_at, coalesce(refund.business_date,(refund.created_at at time zone 'Australia/Brisbane')::date) transaction_date,
      store.store_code, store.store_name, sales_order.invoice_number, sales_order.order_code,
      refund.refund_code, refund.reason, sales_order.customer_name, sales_order.customer_phone,
      sales_order.customer_email, refund.staff_name, refund.method payment_method, sales_order.payment_status,
      sales_order.total invoice_total, sales_order.amount_paid,
      greatest(sales_order.total-sales_order.amount_paid,0) balance_due,
      case when sales_line.line_type='repair' then 'repair' when sales_line.line_type='used_device' then 'mis'
        when sales_line.line_type in ('product','retail') then 'product' else 'other' end category_key,
      sales_line.id line_id, sales_line.name item_name, sales_line.sku, refund_line.returned_quantity quantity,
      sales_line.unit_price, -refund_line.amount amount, coalesce(repair_ticket.ticket_code,'') ticket_code,
      coalesce(nullif(trim(sales_line.line_payload->>'note'),''),nullif(trim(sales_line.line_payload->>'notes'),''),
        nullif(trim(sales_line.line_payload->>'description'),''),'') line_note
    from public.pos_sales_refunds refund join selected_stores store on store.id=refund.store_id
    ...
    join public.pos_sales_order_lines sales_line on sales_line.id=refund_line.sales_order_line_id
    left join public.pos_repair_tickets repair_ticket on repair_ticket.id=sales_line.repair_ticket_id
    where coalesce(refund.business_date,(refund.created_at at time zone 'Australia/Brisbane')::date) between from_value and to_value
  ),
  filtered_lines as materialized (
    select * from sale_line_movements where category_value='all' or category_key=category_value
    union all select * from refund_line_movements where category_value='all' or category_key=category_value
  ),
  grouped_movements as materialized (
    select event_type,event_key,max(transaction_at) transaction_at,max(transaction_date) transaction_date,
      store_code,store_name,invoice_number,order_code,max(refund_code) refund_code,max(reason) reason,
      max(customer_name) customer_name,max(customer_phone) customer_phone,max(customer_email) customer_email,
      max(staff_name) staff_name,max(payment_method) payment_method,max(payment_status) payment_status,
      max(invoice_total) invoice_total,max(amount_paid) amount_paid,max(balance_due) balance_due,
      case when count(distinct category_key)=1 then min(category_key) else 'mixed' end category_key,
      string_agg(distinct nullif(ticket_code,''),', ') ticket_codes,sum(amount) amount,
      sum(greatest(quantity,0))::integer item_count,
      jsonb_agg(jsonb_build_object('line_id',line_id,'category',category_key,'name',item_name,'sku',sku,
        'quantity',quantity,'unit_price',unit_price,'amount',amount,'ticket_code',ticket_code,'note',line_note) order by line_id) items,
      lower(concat_ws(' ',invoice_number::text,order_code,max(refund_code),max(customer_name),max(customer_phone),
        max(customer_email),max(staff_name),string_agg(item_name,' '),string_agg(coalesce(sku,''),' '),
        string_agg(coalesce(ticket_code,''),' '))) search_text
    from filtered_lines group by event_type,event_key,store_code,store_name,invoice_number,order_code
  ),
  searched_movements as materialized (
    select * from grouped_movements movement where query_value='' or movement.search_text like '%'||query_value||'%'
  ),
  summary as (
    select round(coalesce(sum(case when event_type='sale' then amount else 0 end),0),2) sales,
      round(abs(coalesce(sum(case when event_type='refund' then amount else 0 end),0)),2) refunds,
      round(coalesce(sum(amount),0),2) net,count(*)::integer transaction_count from searched_movements
  )
  select jsonb_build_object('ok',true,'date_from',from_value,'date_to',to_value,'store_code',store_value,
    'store_name',case when store_value='' then 'All Stores' else coalesce((select min(store_name) from selected_stores),store_value) end,'category',category_value,
    'query',query_value,'limit',limit_value,'offset',offset_value,
    'has_more',summary.transaction_count>offset_value+limit_value,
    'summary',jsonb_build_object('sales',summary.sales,'refunds',summary.refunds,'net',summary.net,'transaction_count',summary.transaction_count),
    'rows',coalesce((select jsonb_agg(jsonb_build_object('event_type',page.event_type,'event_key',page.event_key,
      'transaction_at',page.transaction_at,'transaction_date',page.transaction_date,'store_code',page.store_code,
      'store_name',page.store_name,'invoice_number',page.invoice_number,'order_code',page.order_code,
      'refund_code',page.refund_code,'reason',page.reason,'customer_name',page.customer_name,
      'customer_phone',page.customer_phone,'customer_email',page.customer_email,'staff_name',page.staff_name,
      'payment_method',page.payment_method,'payment_status',page.payment_status,'invoice_total',page.invoice_total,
      'amount_paid',page.amount_paid,'balance_due',page.balance_due,'category',page.category_key,
      'ticket_codes',coalesce(page.ticket_codes,''),'item_count',page.item_count,'amount',round(page.amount,2),
      'items',page.items) order by page.transaction_at desc,page.event_key desc)
      from (select * from searched_movements order by transaction_at desc,event_key desc limit limit_value offset offset_value) page),'[]'::jsonb)
  ) into result_payload from summary;
  return result_payload;
end;
$$;
revoke all on function public.get_admin_sales_drilldown(text,date,date,text,text,text,integer,integer) from public,anon,authenticated;
grant execute on function public.get_admin_sales_drilldown(text,date,date,text,text,text,integer,integer) to anon,authenticated,service_role;
