...
- Payment never closes a repair card automatically. After full payment, staff explicitly keep the card in an open status or finish it. That choice also lives on the card itself, so a card paid on an earlier day, or one whose post-payment dialog was dismissed, can still be closed. The card names what is holding it open and disables the control until nothing is.
