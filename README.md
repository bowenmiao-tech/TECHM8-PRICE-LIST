...
Used-device access and publishing checks: `node tests/used-device-buyback.cjs` and the rollback-only SQL checks in `supabase/tests/used_device_*.sql` (including `used_device_sale_test.sql`) and `supabase/tests/buyback_intake_rework.sql`, plus `node tests/admin-used-device-stock.cjs` for the admin page (website listings run against the website project). Device creation/updates and inventory/transaction reads authorize the store inside the RPC; operation attribution follows `pos_authorized_actor`. Staff can enter refurbishment costs but cannot read historical or cumulative costs. Admin sessions retain cost-history access. The five-minute publish worker calls `net.http_post`; `used_device_sale_return.sql` verifies sale, duplicate-sale rejection and full-refund return to inspection without retaining test stock or invoices.

New repair intake now saves the repair ticket and the customer's first signature in one database transaction. A rejected signature, invalid customer, expired employee session, or wrong store rolls back the whole operation and the POS shows the exact rejection instead of creating a local-only card. Intake photos are compressed in the browser and uploaded separately after that transaction, with the existing ticket-photo retry queue. Special repairs still allow an intentional `$0` price. Deploy `20260912013126_atomic_repair_ticket_signature.sql` and the updated `pos-repair-cards` Edge Function before publishing `pos.html`. Validation: `node tests/repair-ticket-creation.cjs`, `node --experimental-strip-types tests/repair-card-edge.mjs`, and the rollback-only `tests/repair-ticket-atomic-create.sql`.
...
The initial repair ticket and signed intake card are created atomically. Permanent backend rejections are shown verbatim and are not cached as pending local tickets. Intake photos are limited to 10, compressed before submission, and uploaded as separate repair updates after the ticket/signature transaction; raw image data is never stored in the main repair-ticket JSON. The signature screen also states whether terms acknowledgement, a typed customer name, or a signature is still missing.
...
- The intake form runs device first: category and the model search, then the resolved brand/model/colour, then storage and battery health, then the IMEI or serial number. Condition, sale price, the IMEI check reference and the inventory status are not asked for at the counter -- none of them can be answered honestly while the seller is waiting. A device is saved unpriced and graded `Good`, or `Faulty` when a check failed, and is graded and priced at the listing step.
- Seller name, address, ID reference, identity confirmation, ownership, declaration, payout method, and staff/store audit are mandatory. Phone and email are optional; the ID sighted is what identifies the seller, and the repeat-seller alert counts visits by ID for everyone and by phone where one was given. How the device was obtained is an optional note; the seller declaration confirms identity was sighted and matches the recorded ID, not an age of 18 or over.
- The seller is recorded as a customer. `create_pos_used_device_acquisition` finds or creates the `pos_customers` row through `pos_link_buyback_customer` -- matched on phone number, then email -- and never renames an existing customer: only contact details the record is missing are filled in. A seller who leaves neither is not linked, rather than creating a duplicate customer on every visit. The customer history panel then shows sales on the left and buybacks on the right.
- Buybacks have their own per-store number, starting at `1`, held in `pos_store_buyback_counters` the same way invoice numbers are. The number is what the seller is quoted; it is shown on the device record and in the customer's buyback list. The buyback list carries no payout amount, because what the shop paid is admin-only.
- A cash payout records nothing further. A bank transfer must say where the money went: a PayID (mobile number or email), or a BSB, account number and account name. The POS and the RPC both refuse a transfer without it.
...
- The inspection checklist follows the paper TECHM8 inspection list item for item and switches with the category. Phone and Tablet share one 21-point list: the nineteen checks on the paper sheet plus wireless charging and the camera button. Power and boot, housing, battery and liquid damage are not on that sheet and are no longer asked at intake; battery health has its own field. Laptop, Watch, Game Console, and Other keep their own lists. Checks stored against older records stay visible even when they are not in the current list.
...
- What a device was like when it was bought is a locked record. The purchase inspection, battery health and condition captured at the counter (`inspection`, `intake_battery_health`, `intake_condition_grade`) are guarded by the `pos_used_devices_lock_intake_record` trigger, so no code path, including the admin portal, can rewrite them. The current condition, battery and colour stay correctable as listing attributes.
- Whether a device can be sold comes from a separate pre-sale test, run once it has been cleaned up or repaired. Runs live in `pos_used_device_sale_tests`, append-only; each is a complete run of today's checklist, and the latest one decides. A battery reading taken during the test becomes the current battery; the purchase-time reading stays. A listed device that fails a retest is taken off sale in the same step. The public listing's "N of N checks passed" reads the pre-sale test, not the purchase inspection.
- The pre-sale test page is one shared module, `used-device-sale-test.js`, mounted in the POS device detail and in the admin portal, reading and writing through `get_pos_used_device_sale_tests` and `record_pos_used_device_sale_test` (staff for their own store, admin for any).
- A passing pre-sale test, IMEI status, activation-lock removal, data erasure, a listing photo and a sale price above zero gate `Ready for sale`.
- A purchase cannot be saved without at least one intake photo, taken while the form is open and claimed by the acquisition in the same transaction that pays the seller. Every purchase starts in `Inspection`; reaching the shelf additionally needs at least one listing photo. Devices bought before this rule carry `evidence_required = false` and are unaffected.
- Evidence and the running memo live in `pos_used_device_update_records`, staged as intake / seller ID / refurbishment / listing, and are read through the `pos_used_device_updates` view of the entries that have not been removed.
- A photo taken by mistake can be removed by staff or the admin from any of the four tabs; staff never see a saved seller ID photo, so only the admin can remove one. Removal takes the photo out of every view and off the website, but the row and the file are kept, marked with who removed it and when, and the device history records it (`photo_removed`). A purchase must keep at least one intake photo and a device on sale at least one listing photo, so the right photo is added first. Removing a listing photo from a device on sale republishes it. Photos on the buy form before the purchase is saved are drafts and are deleted outright, file included. Server side: `remove_pos_used_device_photo`, `remove_pos_used_device_intake_upload`, and the `remove` action of `pos-used-device-updates`. Seller identity images are kept in their own private bucket, are readable only through an admin session, and every read is recorded in `pos_used_device_id_photo_views`.
- What a device cost the business is admin-only. A staff session receives no purchase price, refurbishment total, total cost or payout: they are stripped from `pos_used_device_payload`, the workspace summary, and the transaction register, and the below-cost refusal no longer quotes the figure it is protecting. Staff still enter a purchase price while paying a seller, and still record refurbishment costs for the work they do; they just cannot read the numbers back. The admin portal reads the tables directly and is unaffected.
- Refurbishment is recorded per device in `pos_used_device_costs`. Staff record what was repaired or replaced, with no amount; the admin portal prices each line (`save_admin_used_device_cost`), and staff never see an amount. Every margin figure -- the admin device detail, the admin overview -- uses purchase price plus the priced lines. Pricing a device below what it has cost is still possible but requires a recorded reason, which is what the ledger and the admin alert read. Recording a part here does not move product inventory; product stock lives in the separate website project.
...
- Payment never closes a repair card automatically. After full payment, staff explicitly keep the card in an open status or finish it. A `Finish card` button sits beside the status selector on the card, so a card paid on an earlier day, or one whose post-payment dialog was dismissed, can still be closed. It is disabled until the card actually qualifies, and its tooltip names what is still holding the card open. Closing from there records the resolution as repaired.
- Invoicing a repair does not finish it. A repair billed while still proposed, approved, or in progress keeps its own status, shows it next to `Invoiced`, and offers a Mark complete control. Its name, price, and approval stay locked because the customer has already paid for them, and cancelling invoiced work belongs in the refund flow, so the control only moves the status to completed.
...
- Going ready for sale publishes, a price change republishes, and selling, returning to the seller, disposal or a move back to inspection takes the listing down. In the POS, "Put on Sale & Website" does the whole step for a finished device: with a passing pre-sale test, the lock, data and stolen checks done, a listing photo and a sale price, one press marks it ready for sale and sends it to the website. Until then the panel lists exactly what is still missing, from the same `pos_used_device_listing_blockers` the admin portal uses.
- The public listing text: battery health is shown only from 85% up and reads "Good battery" below that, so the number never leaves the POS; there is no inspection count and no "In stock at" line, because the store is sent on its own and shown as the device location; the title drops the repeated product line ("Apple iPhone 17 Pro", not "Apple iPhone iPhone 17 Pro").
- On the website each published device is also a product in the shop, under Second Hand Devices (Used Phones, Used Tablets, ...), so customers add it to the normal cart and pay online or in store. It is one of a kind: its slug starts with `used-`, and the cart and checkout never take more than one. Its page is `used-device.html?d=<slug>`, reading the live catalogue on every visit. The POS website panel links to it once it is live.
- A website order reserves the device before the customer pays. Checkout asks this project (`used-device-online-orders`, `hold_pos_used_devices_online`); if the device is no longer for sale or another order has it, the website refuses the order. While reserved, the device shows "Reserved online" in the POS, the Sell button is off, the counter sale is refused, and its price and status cannot change. A checkout reservation lapses after 45 minutes; one to pay in store lasts until the website order is paid or cancelled, and staff take that payment in the website admin, not at the till.
- The website is the record of how the order went. Every five minutes while anything is reserved, `used-device-online-sync` asks it (`apply_pos_used_device_online_sync`): a paid order becomes a sale here, on the device's history with the website order code and amount, and the listing comes down; an abandoned or cancelled order puts the device back on sale. The admin portal shows the website order on the sold device instead of an invoice number.
...
- `admin.html` > `Used Devices` opens on two lists, For Sale and Sold, each showing the store the device is in, with a store filter and search. Returned or written-off devices sit behind a third tab that only appears when there are any.
- A device opens on four pages: Price and listing (set the price and "Confirm and put it online", with a live margin against purchase plus refurbishment), Pre-sale test, At purchase (the locked record, the seller and the payout destination), and History and photos. The confirm button stays disabled, with the reasons listed, until nothing stands between the device and the website; `pos_used_device_listing_blockers` is the single definition of those reasons.
- Confirming calls `set_admin_used_device_listing`, which records the price and approval on the ledger under the admin's name; the existing website trigger queues the publish and the page pushes it straight away through `pos-used-device-publish`, falling back to the five-minute drain if that push fails.
- Payouts and stock per store, the exception list, POS-against-daily-report reconciliation and the second-hand dealer register with its CSV export are under Compliance and audit on the same page.
...
Buyback inspection uses direct Pass / Fail / N/A choices like the repair intake template. Background inventory refreshes preserve the open buyback form. Deploy `20260913104624_simplify_buyback_intake.sql` with the updated POS and evidence script; verification covers one-photo acquisition and photo linkage.

The reworked intake form ships as `20260918120000_rework_buyback_intake_form.sql` with the updated `pos.html` and the `pos-shared-state` edge function (it gains the `customer-buybacks` read). The migration rewrites the phone and tablet checklist, numbers existing purchases per store, links past sellers to customers by phone number, and relaxes the sale-price rule so a device can be bought before it is priced. Verification is `node tests/used-device-buyback.cjs`.

The admin device lists ship as `20260918140000_add_admin_used_device_stock.sql` (with its follow-up `20260918141500_fix_used_device_listing_blockers_append.sql`), and the locked purchase record with the pre-sale test as `20260920090000_lock_intake_and_add_pre_sale_test.sql`, together with `used-device-sale-test.js` and `.css`, `admin.html` and `pos.html`. Verification: `node tests/admin-used-device-stock.cjs` (desktop, both iPad orientations and a phone), `node tests/used-device-buyback.cjs`, and the rollback-only `supabase/tests/used_device_sale_test.sql`. One-press publishing, the listing text and admin-only repair pricing ship as `20260921140000_publish_from_pos_and_admin_refurb_amounts.sql`; its rollback-only check is `supabase/tests/used_device_publish_flow.sql`. Selling used devices through the website shop ships as `20260921170000_reserve_used_devices_for_website_orders.sql` here and `website-migrations/20260921170000_sell_used_devices_in_the_shop.sql` in the website project, with the new `used-device-online-orders` function, the redeployed `pos-used-device-publish` and `used-device-listings`, and the website's `create-checkout-session` and `submit-order`; the rollback-only checks are `supabase/tests/used_device_online_orders.sql` and `supabase/tests/website_used_device_shop_orders.sql`. Photo removal ships as `20260921100000_allow_removing_used_device_photos.sql` with the redeployed `pos-used-device-updates` function and `used-device-evidence.js`/`.css`; its rollback-only check is `supabase/tests/used_device_photo_removal.sql`.

Used-device Transfers now includes a device/store picker. Receipt requires at least one photo from the destination store and atomically moves the device, claims its evidence, and records the receiving staff member. The prior sale status is restored on receipt; retrying confirmation does not duplicate the ledger. Admin Used Devices → device → History shows transfer routes and receipt photos. Deploy `20260921151427_complete_used_device_transfers.sql`, `pos.html`, `admin.html`, and `used-device-evidence.js` together. Checks: `node tests/used-device-transfers.cjs`, `node tests/admin-used-device-stock.cjs`, `node tests/used-device-buyback.cjs`, and rollback-only `supabase/tests/used_device_transfers.sql`.

Admin Used Devices detail has a Delete device action. `delete_admin_used_device` requires an admin session and exact device-code confirmation; it permanently removes the device and dependent operational records, including its signed document record. Existing sales invoice lines keep their descriptions and amounts with the device FK detached. Active website orders block deletion, and a live listing must finish withdrawal first. The admin UI handles withdrawal before deletion. Deploy `20260922114239_add_admin_used_device_delete.sql` and `admin.html`; verify with `supabase/tests/admin_used_device_delete.sql` and `node tests/admin-used-device-stock.cjs`.
