...
Used-device access and publishing checks: `node tests/used-device-buyback.cjs` and the rollback-only SQL checks in `supabase/tests/used_device_*.sql` (website listings run against the website project). Device creation/updates and inventory/transaction reads authorize the store inside the RPC; operation attribution follows `pos_authorized_actor`. Staff can enter refurbishment costs but cannot read historical or cumulative costs. Admin sessions retain cost-history access. The five-minute publish worker calls `net.http_post`; `used_device_sale_return.sql` verifies sale, duplicate-sale rejection and full-refund return to inspection without retaining test stock or invoices.
