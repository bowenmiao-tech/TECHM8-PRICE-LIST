...
- The public listing text: battery health is shown only from 85% up and reads "Good battery" below that, so the number never leaves the POS; the inspection line counts only the checks that apply ("13 of 13", not "13 of 21" for a tablet); the title drops the repeated product line ("Apple iPhone 17 Pro", not "Apple iPhone iPhone 17 Pro").
- The storefront pages are in the website repository: `used-devices.html` (the category list) and `used-device.html?d=<slug>` (one device), both reading the live catalogue on every visit so a device appears when it is published and disappears when it sells. The POS website panel links to the device's page once it is live.
