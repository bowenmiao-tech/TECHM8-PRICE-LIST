-- Staff accounts may stay signed in on multiple devices at the same time.
-- Changing first-login credentials updates the account but keeps every other
-- still-valid device session. Explicit logout continues to revoke only the
-- token from the device performing the logout.
...
comment on function public.create_staff_session(text, text) is
  'Creates an independent staff session. The same staff account may have multiple active device sessions.';

comment on function public.change_staff_credentials(text, text, text) is
  'Completes first-login credentials without revoking the account''s other active device sessions.';

comment on function public.revoke_staff_session(text) is
  'Revokes only the supplied device session token.';
