create table if not exists public.nl_monthly_charges (
  id bigint generated by default as identity primary key,
  store_id bigint not null references public.store_locations(id),
  charge_month date not null,
  rent_inc_gst numeric(12,2) not null default 0,
  internet_inc_gst numeric(12,2) not null default 0,
  mobile_inc_gst numeric(12,2) not null default 0,
  pos_inc_gst numeric(12,2) not null default 0,
  franchise_fee_inc_gst numeric(12,2) not null default 0,
  notes text not null default '',
  status text not null default 'draft',
  uploaded_at timestamptz,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint nl_monthly_charges_store_month_unique unique (store_id, charge_month),
  constraint nl_monthly_charges_month_start_check
    check (charge_month = date_trunc('month', charge_month)::date),
  constraint nl_monthly_charges_status_check check (status in ('draft', 'uploaded')),
  constraint nl_monthly_charges_amounts_check check (
    rent_inc_gst >= 0
...
create index if not exists nl_monthly_charges_month_idx
  on public.nl_monthly_charges (charge_month desc, store_id);

drop trigger if exists nl_monthly_charges_set_updated_at on public.nl_monthly_charges;
create trigger nl_monthly_charges_set_updated_at
before update on public.nl_monthly_charges
for each row execute function public.set_updated_at();

alter table public.nl_monthly_charges enable row level security;

drop policy if exists nl_monthly_charges_no_direct_access on public.nl_monthly_charges;
create policy nl_monthly_charges_no_direct_access
on public.nl_monthly_charges
for all
to anon, authenticated
using (false)
with check (false);

revoke all on table public.nl_monthly_charges from anon, authenticated;
...
begin
  ...
  exception when others then
    raise exception 'Monthly charge amounts must be valid numbers';
  end;

  if rent_value < 0 or internet_value < 0 or mobile_value < 0
    or pos_value < 0 or franchise_value < 0 then
    raise exception 'Monthly charge amounts cannot be negative';
  end if;
...
create or replace function public.upload_nl_monthly_charge(
  session_token text,
  target_month text
)
returns jsonb
...
begin
  ...
  month_value := date_trunc('month', nullif(target_month, '')::date)::date;
  ...
  if not found then
    raise exception 'Save the monthly charges before uploading';
  end if;
  if selected_charge.status = 'uploaded' then
    raise exception 'This month has already been uploaded';
  end if;
  ...
  returning * into selected_charge;
...
begin
  ...
  select coalesce(jsonb_agg(
    ...
    || jsonb_build_object(
      'uploaded_product_payable_inc_gst',
      coalesce((
        select round(sum(
...
revoke all on function public.upload_nl_monthly_charge(text, text) from public;
...
grant execute on function public.upload_nl_monthly_charge(text, text) to anon, authenticated;
