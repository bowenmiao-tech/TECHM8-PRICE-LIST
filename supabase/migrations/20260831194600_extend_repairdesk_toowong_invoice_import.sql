...
begin
  ...
  if position('invoice_number_value > 3286' in function_definition) > 0 then
    function_definition := replace(
      function_definition,
      'invoice_number_value > 3286',
      'invoice_number_value > 3848'
    );
    execute function_definition;
  elsif position('invoice_number_value > 3848' in function_definition) = 0 then
    raise exception 'Unable to identify the approved RepairDesk invoice range';
  end if;
end;
$migration$;
...
comment on function public.import_repairdesk_sales_batch(jsonb) is
  'Idempotently imports approved TechM8 Toowong RepairDesk invoices 1-3848 without changing inventory or active shifts.';
...
begin
  for source_customer in
    with raw_customers as (
      ...
      where store_location.store_code = 'toowong'
...
comment on function public.sync_repairdesk_toowong_invoice_customers() is
  'Creates or enriches searchable POS customer records from protected Toowong RepairDesk invoice history.';
